...
class AliPay(object):
    ...
    def __init__(self, appid, app_notify_url, app_private_key_path=None, app_private_key_string=None, alipay_public_key_path=None,
        alipay_public_key_string=None, sign_type="RSA2", debug=False):
        """
        初始化:
        alipay = AliPay(
          appid="",
        ...
        if sign_type not in ("RSA", "RSA2"):
            print "Unsupported sign type "+sign_type
    ...
    def verify(self, data, signature):
        if "sign_type" in data:
            ...
            if sign_type != self._sign_type:
                print "Unknown sign type: "+sign_type
        # 排序后的字符串
        unsigned_items = self._ordered_data(data)
    ...
    def api_alipay_trade_pay(self, out_trade_no, scene, auth_code, subject, notify_url=None, **kwargs):
        """
        eg:
