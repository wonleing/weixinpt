...
def responseMsg(request):
    ...
    if msg['MsgType'] == 'event':
        if msg['Event'] == 'subscribe':
            return share.getMultiXml(msg, '', TaobaoHxfMain.objects.all().order_by('id'))
    ...
    if msg['MsgType'] == 'text':
        if msg['Content'].isdigit():
            try:
                ret = TaobaoHxfMain.objects.get(id=int(msg['Content']))
            except:
                return share.getMultiXml(msg, '', TaobaoHxfMain.objects.all().order_by('id'))
            if ret.type == 0:
                ...
                list = eval("TaobaoHxf%s" %ret.id).objects.all().order_by('id')[:9]
            ...
            if msg['Content'].split("：")[0] == '上传':
                ...
                TaobaoHxfMsg(contact=msg['FromUserName'], content=msg['Content'].split("：")[1]).save()
                return share.getTextXml(msg, u"您的留言已经被接受")
            else:
                return share.getTextXml(msg, u'关键词输入错误')
        else:
            return share.getTextXml(msg, share.randomreply(TaobaoHxfMain))
    elif msg['MsgType'] == 'image':
        newid = TaobaoHxf7.objects.count()+1
        global utitle
        ...
        TaobaoHxf7(id=newid,title=newtitle,description=u'上传人：%s\x0a上传时间：%s' %(uploader, msg['CreateTime']), \
...
def subpage(request, pageid):
    try:
        items = eval("TaobaoHxf%s" %pageid).objects.all().order_by('id')
        cates = TaobaoHxfMain.objects.all().order_by('id')
    except:
    ...
    return render(request, 'wap/zhongmi.html', context)
